 ...
 * Baseado no DistributorImportDialog de módulos.
 */
...
export function InversorImportDialog({ open, onOpenChange, existingInversores }: Props) {
  ...
  const existingKeys = useMemo(
    () => new Set(existingInversores.map(i => `${i.fabricante}|${i.modelo}`.toLowerCase())),
    [existingInversores]
  );

  const newInversores = useMemo(() => {
    if (!parseResult) return [];
    return parseResult.inversores.filter(
  ...
  const duplicateCount = useMemo(() => {
    ...
    return parseResult.inversores.length - newInversores.length;
  }, [parseResult, newInversores]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const result = parseDistributorInversorCSV(text);
  ...
  const handleImport = async () => {
    if (newInversores.length === 0) return;
    setImporting(true);
    setProgress(0);
    ...
    try {
      ...
      const itemsToInsert = newInversores.map(inv => ({
      ...
      toast({
        title: `${imported} inversores importados`,
        ...
        <DialogHeader className="flex flex-row items-center gap-3 p-5 pb-4 border-b border-border shrink-0">
          ...
          <div className="flex-1">
            <DialogTitle className="text-base font-semibold text-foreground">
              Importar Inversores — CSV Distribuidora
            </DialogTitle>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-lg border border-border p-3 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">Inversores detectados</p>
                  </div>
                  <div className="rounded-lg border border-border p-3 text-center">
                    <p className="text-2xl font-bold text-success">{newInversores.length}</p>
                ...
                <div className="max-h-[280px] overflow-auto border border-border rounded-lg">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {newInversores.slice(0, 50).map((inv, i) => (
                        <tr key={i} className="border-t border-border">
                          ...
                          <td className="px-2 py-1">
                            <Badge variant="outline" className="text-xs">{inv.tipo}</Badge>
                          </td>
                          <td className="px-2 py-1">
                            {inv.potencia_nominal_kw === 0 ? (
        ...
        <DialogFooter className="flex justify-end gap-2 p-4 border-t border-border bg-muted/30 shrink-0">
          {importResult ? (
            ...
            <>
              ...
              <Button onClick={handleImport} disabled={newInversores.length === 0 || importing} className="gap-2">
                {importing ? (
                  <><Loader2 className="w-4 h-4 animate-spin" /> Importando...</>
                ) : (
                  `Importar ${newInversores.length} inversores`
                )}
              </Button>
            </>
          ) : (
