...
import type { Conversation, WhatsAppTag } from "@/hooks/useWhatsAppInbox";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => { onLink(null); onOpenChange(false); }}>
            Desvincular
          </Button>
          <Button onClick={() => { onLink(leadId || null); onOpenChange(false); }} disabled={!leadId}>
            Vincular
          </Button>
        </DialogFooter>
      </DialogContent>
