    ...
    switch (block.type) {
      ...
      default: {
        if (isSemanticProposalBlock(block.type)) {
          ...
          return (
            <div className="group rounded-xl border border-border bg-card hover:border-primary/40 hover:shadow-sm transition-all overflow-hidden">
              <div className="flex items-start gap-4 p-5">
                <div className="h-11 w-11 rounded-lg bg-primary/10 flex items-center justify-center text-2xl shrink-0">
                  {meta?.icon ?? "📦"}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {meta?.preview && (
                    <div className="mt-3 px-3 py-2 rounded-md bg-muted/40 border border-dashed border-border/60 text-[11px] text-muted-foreground">
                      <span className="font-medium text-foreground/70">Aparece na proposta como:</span>{" "}
                      {meta.preview}
                    </div>
                  )}
                </div>
        ...
        return <div className="p-4 bg-muted/20 rounded text-xs text-muted-foreground">{block.type}</div>;
      }
    }
  };
