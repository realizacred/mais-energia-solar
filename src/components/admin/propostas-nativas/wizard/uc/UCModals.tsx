...
export function RateioCreditsModal({ open, onOpenChange, ucs, onSave }: RateioProps) {
  ...
  const handleSave = () => {
    const updated = ucs.map((u, i) => ({ ...u, rateio_creditos: values[i] || 0 }));
    onSave(updated);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Rateio Total: <span className="text-secondary">{total}%</span></DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {ucs.map((uc, i) => (
                <div key={uc.id} className="space-y-1">
                  <Label className="text-xs">{i + 1}. {i === 0 ? "(Geradora)" : "Unidade"}</Label>
                  <div className="relative">
                    <Input
                      type="number"
                      min={0}
