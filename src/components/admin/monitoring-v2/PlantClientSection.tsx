...
import { Users, Link as LinkIcon, Unlink, Search } from "lucide-react";
...
export function PlantClientSection({ plantId, clientId, clientName }: Props) {
  ...
  const { data: clientes = [] } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("clientes").select("id, nome, telefone, cidade, estado").eq("ativo", true).order("nome").limit(20);
      if (search.length >= 2) {
        query = query.ilike("nome", `%${search}%`);
      }
      const { data } = await query;
      return (data || []) as Array<{ id: string; nome: string; telefone: string; cidade: string | null; estado: string | null }>;
    },
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {[c.cidade, c.estado].filter(Boolean).join(" - ") || c.telefone}
                        </p>
                      </div>
