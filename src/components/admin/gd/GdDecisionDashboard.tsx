 ...
 * Shows rateio summary, analysis table, optimization suggestions, and comparison.
...
import {
  ...
  Lightbulb, ArrowRight, BarChart3, Percent, Zap, Scale
} from "lucide-react";
import {
...
export function GdDecisionDashboard({ generationKwh, generatorUc, beneficiaries, onApplySuggestion }: Props) {
  ...
  if (generationKwh <= 0 || participants.length === 0) {
    return (
      <div className="text-center py-8 space-y-2 rounded-lg border border-dashed border-border">
        ...
        <p className="text-xs text-muted-foreground">É necessário ter geração e beneficiárias cadastradas</p>
      </div>
    );
  }
