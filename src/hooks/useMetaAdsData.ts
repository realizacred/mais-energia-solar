  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("ad_id, ad_name, campaign_id, campaign_name, spend, clicks, impressions, leads_count, ctr, cpc, cpl")
        .gte("date", since)
      ...
      // Aggregate ads
      const adsMap = aggregateBy(rows, "ad_id", "ad_name", ["spend", "clicks", "impressions", "leads_count"]);
      ...
      // Aggregate campaigns
      const campMap = aggregateBy(rows, "campaign_id", "campaign_name", ["spend", "clicks", "impressions", "leads_count"]);
