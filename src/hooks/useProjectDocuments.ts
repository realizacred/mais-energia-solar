...
export function useUploadProjectDocument() {
  ...
  return useMutation({
    ...
    onSuccess: async (data: any, variables) => {
      ...
      // Automação de transição de etapa por documento
      const { dealId, categoria } = variables;
      const tenantId = (data as any)?.tenant_id;
      const userId = (data as any)?.uploaded_by;

      if (!dealId || !categoria || !tenantId) return;

      const isHomologacao = categoria === "Parecer de Acesso" || categoria === "Homologação aprovada";
      const isART = categoria === "ART";

      if (!isHomologacao && !isART) return;

      try {
        // 1. Buscar memberships técnicos (Engenharia, Instalação, etc)
        ...
        // Filtrar funis técnicos (Engenharia ou Instalação)
        const techKeywords = ["engenharia", "instalação", "execução"];
        const techMemberships = memberships.filter((m: any) =>
          techKeywords.some((kw) => m.pipelines.name.toLowerCase().includes(kw)),
        );

        if (techMemberships.length === 0) return;

        for (const membership of techMemberships) {
          const pipelineId = membership.pipeline_id;
          const currentStageId = membership.stage_id;

          // 2. Buscar todas as etapas deste funil ordenadas
          ...
          if (stagesErr || !allStages) continue;
          ...
          if (isHomologacao) {
            ...
            // Mover para "Aguardando Vistoria"
            const vistoriaStage = allStages.find((s) =>
              s.name.toLowerCase().includes("vistoria") || s.name.toLowerCase().includes("aguardando vistoria"),
            );
            if (vistoriaStage && vistoriaStage.id !== currentStageId) {
              targetStageId = vistoriaStage.id;
              targetStageName = vistoriaStage.name;
            }
          }
          ...
          if (targetStageId) {
            ...
            if (!updErr) {
              // 4. Registrar no histórico (project_events)
              const currentStageName = allStages.find(s => s.id === currentStageId)?.name || "Anterior";
              
              await supabase.from("project_events").insert({
                tenant_id: tenantId,
                deal_id: dealId,
                event_type: "stage_changed",
                from_value: currentStageName,
                to_value: targetStageName,
                actor_user_id: userId,
                metadata: {
                  reason: `Automação via anexo de ${categoria}`,
                  membership_id: membership.id,
                  pipeline_name: (membership as any).pipelines.name,
                  document_category: categoria
                }
              });

              toast({
                title: "Projeto avançado automaticamente",
                description: `Devido ao anexo de ${categoria}, o funil ${(membership as any).pipelines.name} foi movido para ${targetStageName}.`,
              });
            }
          }
        }
      } catch (err) {
        console.error("[useUploadProjectDocument] erro na automação de etapa:", err);
      }
    },
    onError: (e: any) => {
