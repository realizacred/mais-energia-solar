  ...
  return useQuery<WaHealthMetrics>({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("wa_messages").select("id", { count: "exact", head: true }).eq("direction", "inbound").gte("created_at", since24h),
        supabase.from("wa_messages").select("id", { count: "exact", head: true }).eq("direction", "outbound").gte("created_at", since24h),
        supabase.from("wa_messages").select("id", { count: "exact", head: true }).eq("direction", "inbound").gte("created_at", since7d),
        supabase.from("wa_messages").select("id", { count: "exact", head: true }).eq("direction", "outbound").gte("created_at", since7d),
