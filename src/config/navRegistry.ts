...
export const NAV_REGISTRY: NavRegistryItem[] = [
  ...
  { nav_key: "sla-breaches", label_default: "Alertas de Prazo", group_default: "Comercial", order_default: 5, icon: "AlertTriangle", description: "Leads com tempo de resposta excedido", keywords: ["prazo", "atraso", "violação", "alerta", "SLA"], criticality: "normal", permission: "admin_only" },
  ...
  { nav_key: "whatsapp", label_default: "Automação WhatsApp", group_default: "Atendimento", order_default: 1, icon: "Bot", description: "Respostas automáticas, boas-vindas e horário comercial", keywords: ["API", "automação", "webhook", "bot", "auto-resposta"], criticality: "normal", permission: "admin_only", separator: true },
  ...
  { nav_key: "followup-queue", label_default: "Fila de Follow-ups", group_default: "Atendimento", order_default: 3, icon: "CalendarClock", description: "Clientes aguardando retorno automático", keywords: ["pendente", "fila", "aguardando", "retorno"], criticality: "normal", permission: "all" },
  { nav_key: "wa-instances", label_default: "Instâncias WhatsApp", group_default: "Atendimento", order_default: 4, icon: "Smartphone", description: "Conexões Evolution API (QR Code)", keywords: ["instância", "evolution", "API", "número", "QR"], criticality: "normal", permission: "admin_only", separator: true },
  { nav_key: "wa-etiquetas", label_default: "Etiquetas WhatsApp", group_default: "Atendimento", order_default: 5, icon: "Tag", description: "Tags para organizar conversas", keywords: ["tag", "etiqueta", "classificação", "organizar", "whatsapp"], criticality: "normal", permission: "admin_only" },
  { nav_key: "respostas-rapidas", label_default: "Respostas Rápidas", group_default: "Atendimento", order_default: 6, icon: "Sparkles", description: "Templates de mensagens pré-definidas", keywords: ["template", "atalho", "mensagem", "rápida"], criticality: "normal", permission: "admin_only" },
  { nav_key: "metricas-atendimento", label_default: "Métricas de Atendimento", group_default: "Atendimento", order_default: 7, icon: "BarChart3", description: "Performance de atendimento por consultor", keywords: ["métricas", "performance", "tempo", "resposta", "SLA", "consultor"], criticality: "normal", permission: "admin_only" },
  ...
  { nav_key: "validacao", label_default: "Validação de Vendas", group_default: "Operações", order_default: 1, icon: "ClipboardCheck", description: "Validar e aprovar vendas realizadas", keywords: ["aprovação", "validar", "conferência"], criticality: "normal", permission: "admin_only" },
  { nav_key: "tarefas", label_default: "Tarefas & SLA", group_default: "Operações", order_default: 2, icon: "ClipboardList", description: "Controle de pendências e prazos", keywords: ["tarefa", "prazo", "SLA", "pendência"], criticality: "normal", permission: "all" },
  ...
  { nav_key: "n8n", label_default: "Automações", group_default: "Integrações", order_default: 3, icon: "Workflow", description: "Workflows via MCP", keywords: ["n8n", "automação", "workflow", "MCP"], criticality: "normal", permission: "admin_only" },
  { nav_key: "google-maps-config", label_default: "Google Maps", group_default: "Integrações", order_default: 4, icon: "MapPin", description: "API Key para mapas e geocodificação", keywords: ["google", "maps", "mapa", "api", "geocodificação"], criticality: "normal", permission: "admin_only" },
  { nav_key: "instagram", label_default: "Instagram", group_default: "Integrações", order_default: 5, icon: "Instagram", description: "Sincronizar posts do Instagram", keywords: ["instagram", "post", "rede social", "feed"], criticality: "normal", permission: "admin_only" },
  ...
  // ── 14. Meta (Marketing / Facebook Ads) ──
  { nav_key: "meta-dashboard", label_default: "Dashboard", group_default: "Meta", order_default: 0, icon: "BarChart3", description: "Métricas de campanhas e performance de anúncios", keywords: ["meta", "facebook", "ads", "dashboard", "métricas", "performance", "marketing"], criticality: "normal", permission: "admin_only" },
  { nav_key: "meta-leads", label_default: "Leads", group_default: "Meta", order_default: 1, icon: "UserPlus", description: "Leads capturados via Facebook Lead Ads", keywords: ["meta", "facebook", "lead", "formulário", "captura"], criticality: "normal", permission: "admin_only" },
  { nav_key: "meta-campaigns", label_default: "Campanhas", group_default: "Meta", order_default: 2, icon: "Megaphone", description: "Campanhas ativas e histórico de investimento", keywords: ["meta", "facebook", "campanha", "anúncio", "investimento", "ad"], criticality: "normal", permission: "admin_only" },
  { nav_key: "meta-config", label_default: "Configuração", group_default: "Meta", order_default: 3, icon: "Settings", description: "Credenciais e conexão com Meta", keywords: ["meta", "config", "token", "credencial"], criticality: "normal", permission: "admin_only" },
  ...
  // ── 18. Projetos & Propostas (configurações) ──
  ...
  { nav_key: "custom-fields", label_default: "Opções Customizáveis", group_default: "Projetos & Propostas", order_default: 1, icon: "Settings2", description: "Campos customizados, premissas, tipos de atividade", keywords: ["campo", "customizado", "atividade", "motivo", "perda", "funil", "personalizar", "custom", "premissa"], criticality: "normal", permission: "admin_only" },
  ...
  { nav_key: "release", label_default: "Release Notes", group_default: "Administração", order_default: 6, icon: "Rocket", description: "Controle de entregas e lançamentos", keywords: ["versão", "release", "novidade"], criticality: "normal", permission: "admin_only" },
...
export const NAV_SECTION_DEFAULTS: NavSectionMeta[] = [
  ...
  { label: "Estoque",               icon: "Package",        order: 6,    indicatorBg: "bg-sidebar-operations",    activeClass: ACTIVE("operations"),    hoverClass: HOVER("operations"),    labelClass: LABEL("operations"),    defaultOpen: false },
  { label: "Financeiro",            icon: "Wallet",         order: 7,    indicatorBg: "bg-sidebar-finance",       activeClass: ACTIVE("finance"),       hoverClass: HOVER("finance"),       labelClass: LABEL("finance"),       defaultOpen: false },
  { label: "Equipe",                icon: "Users",          order: 8,    indicatorBg: "bg-sidebar-cadastros",     activeClass: ACTIVE("cadastros"),     hoverClass: HOVER("cadastros"),     labelClass: LABEL("cadastros"),     defaultOpen: false },
  { label: "IA",                    icon: "Bot",            order: 9,    indicatorBg: "bg-sidebar-ai",            activeClass: ACTIVE("ai"),            hoverClass: HOVER("ai"),            labelClass: LABEL("ai"),            defaultOpen: false },
  { label: "Monitoramento",         icon: "Sun",            order: 10,   indicatorBg: "bg-sidebar-commercial",    activeClass: ACTIVE("commercial"),    hoverClass: HOVER("commercial"),    labelClass: LABEL("commercial"),    defaultOpen: false },
  { label: "Integrações",           icon: "Plug",           order: 11,   indicatorBg: "bg-sidebar-settings",      activeClass: ACTIVE("settings"),      hoverClass: HOVER("settings"),      labelClass: LABEL("settings"),      defaultOpen: false },
  { label: "Site",                  icon: "Globe",          order: 12,   indicatorBg: "bg-sidebar-marketing",     activeClass: ACTIVE("marketing"),     hoverClass: HOVER("marketing"),     labelClass: LABEL("marketing"),     defaultOpen: false },
  { label: "Meta",                  icon: "Facebook",       order: 13,   indicatorBg: "bg-sidebar-marketing",     activeClass: ACTIVE("marketing"),     hoverClass: HOVER("marketing"),     labelClass: LABEL("marketing"),     defaultOpen: false },
  { label: "Cadastros",             icon: "Database",       order: 14,   indicatorBg: "bg-sidebar-cadastros",     activeClass: ACTIVE("cadastros"),     hoverClass: HOVER("cadastros"),     labelClass: LABEL("cadastros"),     defaultOpen: false },
  { label: "Medidores",             icon: "Gauge",          order: 15,   indicatorBg: "bg-sidebar-commercial",    activeClass: ACTIVE("commercial"),    hoverClass: HOVER("commercial"),    labelClass: LABEL("commercial"),    defaultOpen: false },
  { label: "Unidades Consumidoras", icon: "Building2",      order: 16,   indicatorBg: "bg-sidebar-clients",       activeClass: ACTIVE("clients"),       hoverClass: HOVER("clients"),       labelClass: LABEL("clients"),       defaultOpen: false },
  { label: "Projetos & Propostas",  icon: "FolderCog",      order: 17,   indicatorBg: "bg-sidebar-commercial",    activeClass: ACTIVE("commercial"),    hoverClass: HOVER("commercial"),    labelClass: LABEL("commercial"),    defaultOpen: false },
  { label: "Configurações",         icon: "Settings",       order: 18,   indicatorBg: "bg-sidebar-settings",      activeClass: ACTIVE("settings"),      hoverClass: HOVER("settings"),      labelClass: LABEL("settings"),      defaultOpen: false },
  { label: "Administração",         icon: "Shield",         order: 19,   indicatorBg: "bg-sidebar-settings",      activeClass: ACTIVE("settings"),      hoverClass: HOVER("settings"),      labelClass: LABEL("settings"),      defaultOpen: false },
];
