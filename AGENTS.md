...
Cliente (se não existir) - SELECT antes de INSERT
Projeto (com external_source='solarmarket' + external_id)
Deal (OBRIGATÓRIO - sem deal = sem kanban comercial)
Vínculos funil_id + etapa_id (em projeto_funis/projeto_etapas)
...
sm-enrich-versoes ? valor, potência, kit, UCs, localização
...
LER AGENTS.md seção "Migração SM" (RB-52, RB-57, RB-58, RB-59 a RB-75)
VALIDAR se solução proposta viola alguma RB
Se violar: REPORTAR ao usuário antes de implementar
Se não violar: implementar e referenciar RB no commit message
...
Pode criar os 4 padrão (Comercial, Engenharia, Equipamento, Compesação)
...
DA-47 MIGRAÇÃO SM — SCHEMA DUAL FUNIL/PIPELINE É DEFINITIVO
