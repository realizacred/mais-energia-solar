...
-- Data: 2026-02-09 (revisão v2)
-- ================================================================
--
-- MUDANÇAS v1 → v2:
-- 1. Adicionada seção PRÉ-REQUISITOS com:
--    - Função resolve_public_tenant_id()
--    - Triggers de resolução de tenant para leads/orcamentos/simulacoes
--    - ALTER DEFAULT para remover require_tenant_id() dessas tabelas
-- 2. Policies de INSERT anônimo agora exigem tenant_id IS NOT NULL
-- 3. Adicionada policy service_role para audit_logs (trigger de auditoria)
-- 4. Adicionada policy service_role para whatsapp_automation_logs
--
-- ATENÇÃO CRÍTICA:
-- Policies PERMISSIVE usam lógica OR no PostgreSQL.
-- Entre 003 e 004, policies antigas AINDA permitem acesso cross-tenant.
-- Aplicar 004 IMEDIATAMENTE após validar 003. SEM GAP.
-- ================================================================
...
-- Retorna o tenant padrão se existe exatamente 1 tenant ativo.
-- Se 0 ou >1, lança exception (obriga resolução explícita).
...
DECLARE
  _resolved uuid;
BEGIN
  -- 1. Se já tem tenant_id explícito → manter (service_role / admin / trigger)
  IF NEW.tenant_id IS NOT NULL THEN
    RETURN NEW;
  END IF;

  -- 2. Se auth.uid() está disponível → usar get_user_tenant_id()
  ...
  -- 3. Se campo "vendedor" preenchido → resolver via vendedores (codigo ou slug)
...
DECLARE
  _resolved uuid;
BEGIN
  -- 1. Se já tem tenant_id explícito → manter
  IF NEW.tenant_id IS NOT NULL THEN
    RETURN NEW;
  END IF;

  -- 2. Se auth.uid() está disponível
  IF auth.uid() IS NOT NULL THEN
    _resolved := get_user_tenant_id();
    IF _resolved IS NOT NULL THEN
      NEW.tenant_id := _resolved;
  ...
  -- 3. Se lead_id preenchido → herdar tenant do lead
  ...
  -- 4. Se campo "vendedor" preenchido → resolver via vendedores
  IF NEW.vendedor IS NOT NULL AND TRIM(NEW.vendedor) != '' THEN
    SELECT v.tenant_id INTO _resolved
    FROM vendedores v
    WHERE (v.codigo = NEW.vendedor OR v.slug = NEW.vendedor OR v.nome = NEW.vendedor)
...
-- Trocar require_tenant_id() (que falha para anon) por NULL
-- (o trigger resolve o valor correto)
-- ================================================================
...
-- A2: audit_logs (IMUTÁVEL - apenas SELECT via admin + INSERT via trigger)
CREATE POLICY "rls_audit_logs_select_admin"
  ON public.audit_logs FOR SELECT TO authenticated
  USING (tenant_id = get_user_tenant_id() AND is_admin(auth.uid()));
-- Service role para triggers de auditoria (guard_audit_insert já protege contra INSERT direto)
...
CREATE POLICY "rls_whatsapp_automation_logs_all_admin"
  ...
  USING (tenant_id = get_user_tenant_id() AND is_admin(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id() AND is_admin(auth.uid()));
-- service_role para process-whatsapp-automations edge function
...
-- A15: whatsapp_messages (legacy system - admin only + service_role)
CREATE POLICY "rls_whatsapp_messages_all_admin"
  ON public.whatsapp_messages FOR ALL TO authenticated
  USING (tenant_id = get_user_tenant_id() AND is_admin(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id() AND is_admin(auth.uid()));
-- service_role para send-whatsapp-message edge function
...
-- B3-B25: (idêntico ao v1 — omitido por brevidade, ver seção abaixo)
-- IMPORTANTE: Todos os B3-B25 são IDÊNTICOS ao v1.
-- Copiar integralmente do 003 v1 linhas 168-391.
...
-- B20: vendedores (authenticated read + anon read para link público)
...
-- v2: INSERT público com tenant_id IS NOT NULL (resolvido pelo trigger)
...
-- v2: INSERT público com tenant_id IS NOT NULL
...
-- C3-C26: IDÊNTICOS ao v1 (sem mudanças)
-- Copiar integralmente do 003 v1 linhas 486-804.
...
-- C21: instalador_performance_mensal
...
-- C22-C25: vendedor_achievements/metas/metricas/performance_mensal
...
-- ============================================================
-- CLASSE D: TENANT_OWNER_ONLY (IDÊNTICO ao v1)
-- ============================================================
...
-- D5: checklist_instalador_arquivos
...
-- D6: checklist_instalador_respostas
...
-- ============================================================
-- CLASSE E: SERVICE_ONLY + ADMIN_READ (IDÊNTICO ao v1)
-- ============================================================
...
-- E2-E13: solar_market_* (IDÊNTICOS ao v1)
...
-- ============================================================
-- CLASSE F: WHATSAPP_HYBRID (IDÊNTICO ao v1)
-- ============================================================
...
-- F7: whatsapp_conversation_messages
...
-- ============================================================
-- CLASSE G: PUBLIC_INSERT (com tenant_id IS NOT NULL)
-- ============================================================
...
-- ============================================================
-- TABELAS SEM tenant_id (gap documentado — Fase 3)
-- ============================================================
