-- ================================================================
-- MIGRATION 004: REMOVER POLICIES LEGADAS (SEM FILTRO TENANT)
-- Fase 0.2 do Plano de Endurecimento Multi-Tenant
-- Data: 2026-02-09
-- ================================================================
--
-- PRÉ-REQUISITO: Migration 003 já aplicada e validada.
--
-- Esta migration remove TODAS as policies legadas que NÃO filtram
-- por tenant_id, substituídas pelas novas "rls_*" criadas em 003.
--
-- TABELAS PRESERVADAS (policies já corretas):
-- lead_distribution_log, lead_distribution_rules, motivos_perda,
-- site_banners, site_settings, tenants, wa_satisfaction_ratings
--
-- TABELAS SEM tenant_id (policies mantidas até Fase 3):
...
-- solar_market_config (4 policies separadas)
...
-- ============================================================
-- NÃO REMOVER (já corretas ou sem tenant_id):
-- ============================================================
-- lead_distribution_log: distribution_log_insert, distribution_log_select
-- lead_distribution_rules: distribution_rules_admin, distribution_rules_select
-- motivos_perda: motivos_perda_delete/insert/select/update
-- site_banners: Admins manage banners, Super admins manage all banners, Public read active banners
-- site_settings: Admins manage site_settings, Super admins manage all site_settings, Public read site_settings
-- sla_breaches: sla_breaches_insert/select/update (adicionamos delete em 003)
-- tenants: Super admins manage tenants, Admins read own tenant, Public read active tenants
-- wa_satisfaction_ratings: todas as 4 policies
-- wa_conversation_tags: policies mantidas (sem tenant_id até Fase 3)
-- whatsapp_conversation_tags: policies mantidas (sem tenant_id até Fase 3)
-- backfill_audit: policy mantida (tabela interna)
