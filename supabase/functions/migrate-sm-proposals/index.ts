...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (rawBody?.action === "fix_existing_projects") {
      ...
      const { data: allFunis } = await adminClient
        .from("projeto_funis")
        .select("id, nome")
        .eq("tenant_id", tenantId);
      const funisMap = new Map<string, string>();
      for (const f of allFunis || []) {
        funisMap.set(normalizeNameForCompare(f.nome), f.id);
      }
      ...
      const { data: allEtapas } = await adminClient
        ...
        .eq("tenant_id", tenantId)
        .order("ordem", { ascending: true });
      const etapaFirstMap = new Map<string, string>(); // funil_id → first etapa id
      const etapaByNameMap = new Map<string, string>(); // "funil_id::normalizedName" → etapa_id
      for (const e of allEtapas || []) {
        if (!etapaFirstMap.has(e.funil_id)) etapaFirstMap.set(e.funil_id, e.id);
        if (e.nome) {
          const key = `${e.funil_id}::${normalizeNameForCompare(e.nome)}`;
          if (!etapaByNameMap.has(key)) etapaByNameMap.set(key, e.id);
        }
      }
      ...
      let escritorioId = consultorMap.get(normalizeNameForCompare("escritório"));
      if (!escritorioId) {
        const { data: newEscritorio } = await adminClient
          ...
          .select("id")
          .single();
        escritorioId = newEscritorio?.id || null;
        if (escritorioId) consultorMap.set(normalizeNameForCompare("escritório"), escritorioId);
      }

      const engenhariaFunilId = funisMap.get(normalizeNameForCompare("Engenharia")) || null;
      const engenhariaFirstEtapa = engenhariaFunilId ? etapaFirstMap.get(engenhariaFunilId) || null : null;
      ...
      // 4. For each project, find corresponding SM project and fix mapping
      for (const proj of migratedProjects || []) {
        try {
          // Extract sm_project_id from codigo (PROJ-SM-{id} or PROJ-SM-NP-{id})
          ...
          if (!normalizedFunnel || normalizedFunnel === 'vendedores' || normalizedFunnel === 'lead') {
            targetFunilId = engenhariaFunilId;
          } else if (normalizedFunnel.includes('compesa') || normalizedFunnel.includes('compensa')) {
            targetFunilId = funisMap.get(normalizeNameForCompare('Compensação'))
              || funisMap.get(normalizeNameForCompare('Compesação'))
              || engenhariaFunilId;
          } else {
            targetFunilId = funisMap.get(normalizedFunnel) || null;
            if (!targetFunilId) {
              for (const [k, v] of funisMap) {
                if (k.includes(normalizedFunnel) || normalizedFunnel.includes(k)) { targetFunilId = v; break; }
              }
            }
          }
          if (!targetFunilId) targetFunilId = engenhariaFunilId;

          // Resolve etapa_id
          let targetEtapaId: string | null = null;
          if (targetFunilId) {
            // For Vendedores/NULL/LEAD → first etapa
            if (!normalizedFunnel || normalizedFunnel === 'vendedores' || normalizedFunnel === 'lead') {
              targetEtapaId = etapaFirstMap.get(targetFunilId) || null;
            } else {
              // Try matching sm_stage_name
              if (smProj.sm_stage_name) {
                const nameKey = `${targetFunilId}::${normalizeNameForCompare(smProj.sm_stage_name)}`;
                targetEtapaId = etapaByNameMap.get(nameKey) || null;
              }
              // Try all_funnels
              ...
              if (!targetEtapaId) targetEtapaId = etapaFirstMap.get(targetFunilId) || null;
            }
          }
          if (!targetEtapaId) targetEtapaId = engenhariaFirstEtapa;

          // Resolve consultor from Vendedores funnel
          let targetConsultorId: string | null = null;
          // Check all_funnels for Vendedores stage name
          if (Array.isArray(smProj.all_funnels)) {
            for (const f of smProj.all_funnels) {
              ...
              if (fName === "vendedores") {
                ...
                if (sName) {
                  // Check ex-funcionários
                  const EX_FUNC = ['rogerio', 'rogério', 'ricardo'];
                  if (EX_FUNC.includes(sName)) {
                    targetConsultorId = escritorioId || null;
                  } else {
                    targetConsultorId = consultorMap.get(sName) || null;
                    // Partial match
                    if (!targetConsultorId) {
                      for (const [k, v] of consultorMap) {
                        if (k.startsWith(sName) || sName.startsWith(k)) { targetConsultorId = v; break; }
                      }
                    }
                    if (!targetConsultorId) targetConsultorId = escritorioId || null;
                  }
                }
                break;
              }
          ...
          // Fallback: sm_funnel_name = Vendedores
