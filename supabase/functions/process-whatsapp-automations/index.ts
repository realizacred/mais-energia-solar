...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: allowed } = await supabaseRL.rpc("check_rate_limit", {
      _function_name: "process-whatsapp-automations",
      _identifier: identifier,
      _window_seconds: 60,
      _max_requests: 30,
    });
    if (allowed === false) {
      console.warn(`[process-wa-auto] Rate limited: ${identifier}`);
...
// ── Shared resolver import ────────────────────────────
import { resolveWaTemplate } from "../_shared/resolveWaTemplate.ts";

function substituirVariaveis(mensagem: string, dados: Record<string, string>): string {
  return resolveWaTemplate(mensagem, dados);
}
