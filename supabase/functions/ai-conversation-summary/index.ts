...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: tenantRow } = await adminClient.from("tenants").select("status, deleted_at").eq("id", tenantId).single();
    ...
    try {
      ...
      console.warn("[ai-conversation-summary] config fetch failed, using defaults");
    }
