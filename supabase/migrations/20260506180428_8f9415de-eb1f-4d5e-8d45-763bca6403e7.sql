...
BEGIN
  ...
  DELETE FROM projeto_materiais WHERE projeto_id IN (SELECT id FROM projetos WHERE tenant_id = p_tenant_id);
  ...
  DELETE FROM projeto_homologacao WHERE tenant_id = p_tenant_id;

  -- WhatsApp automations vinculadas a clientes (não apaga conversa, só log)
  DELETE FROM whatsapp_automation_logs WHERE tenant_id = p_tenant_id;
  -- Desvincula conversas e cadências dos clientes (cliente_id SET NULL é automático no FK; aqui forçamos limpeza manual)
  UPDATE wa_conversations SET cliente_id = NULL WHERE tenant_id = p_tenant_id AND cliente_id IS NOT NULL;
  UPDATE wa_cadence_enrollments SET cliente_id = NULL WHERE tenant_id = p_tenant_id AND cliente_id IS NOT NULL;

  -- ====== DEALS ======
  DELETE FROM ai_funnel_alerts WHERE tenant_id = p_tenant_id;
  ...
  -- ====== PROJETOS ======
  DELETE FROM projeto_etapas_log WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_documentos WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_anexos WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_observacoes WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_localizacao WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_ucs WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_clientes_link WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_kits WHERE tenant_id = p_tenant_id;
  DELETE FROM projeto_propostas_link WHERE tenant_id = p_tenant_id;
  ...
  -- ====== Unidades consumidoras vinculadas a clientes (mas que não são leads) ======
  -- units_consumidoras tem cliente_id SET NULL automático; preserva UCs livres.
  ...
  -- ====== Reset de contadores e flags SM ======
  UPDATE tenant_counters SET cliente_code = 0, projeto_num = 0, proposta_num = 0
  WHERE tenant_id = p_tenant_id;

  UPDATE sm_classification_v2 SET imported = false, imported_at = NULL, deal_id = NULL, projeto_id = NULL
  WHERE tenant_id = p_tenant_id;
