...
-- Drop and recreate with full tenant safety
DROP FUNCTION IF EXISTS public.create_proposta_nativa_atomic(uuid, jsonb);
...
DECLARE
  ...
  v_versao_id uuid;
BEGIN
  -- 1) Resolve user + tenant (deterministic, no fallback)
  v_user_id := auth.uid();
  ...
  -- If no valid projeto found (null input OR not found/wrong tenant), create one
  ...
  -- 3) Insert propostas_nativas with tenant_id
  ...
  -- 4) Insert first versão with tenant_id
