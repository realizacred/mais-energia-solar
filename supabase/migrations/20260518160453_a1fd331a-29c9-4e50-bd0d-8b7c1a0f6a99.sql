...
BEGIN
  ...
  -- 4. Canonical State Machine (Strict Workflow)
  v_allowed := CASE v_current_status
    WHEN 'draft'     THEN ARRAY['generated', 'cancelled']
    WHEN 'generated' THEN ARRAY['sent', 'cancelled', 'draft']
    WHEN 'sent'      THEN ARRAY['viewed', 'expired', 'cancelled', 'generated'] -- Restricted
  ...
  -- Normalization for current status if stored in PT
  ...
  IF v_current_status = 'enviada'  THEN v_allowed := ARRAY['viewed', 'expired', 'cancelled', 'generated']; END IF;
