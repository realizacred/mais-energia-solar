-- Adicionar o valor 'sustado' ao enum cheque_status se ele não existir
ALTER TYPE public.cheque_status ADD VALUE IF NOT EXISTS 'sustado';

-- Atualizar a função de verificação de dependências para garantir que ela não falhe
-- e use os valores corretos do enum (embora 'sustado' agora seja válido)
...
BEGIN
  ...
  -- 2. Check for payments (Lancamentos)
  ...
  -- This is usually a soft check, but if project is "In Progress" we might lock
  -- 4. Check for Obra
