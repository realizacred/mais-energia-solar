DO $backfill$
DECLARE
  v_total int := 0; v_ok int := 0; v_skipped int := 0; v_errors int := 0;
  v_batch_size int := 50; v_processed_in_batch int;
  v_proposta record; v_result jsonb;
  v_offset int := 0; v_batch_num int := 0;
...
BEGIN
  LOOP
    ...
    FOR v_proposta IN
      SELECT id, codigo FROM public.propostas_nativas
      WHERE status = 'aceita' ORDER BY created_at
    ...
    LOOP
      ...
      BEGIN
        ...
        IF v_result ? 'error' THEN
          ...
          END IF;
        ELSIF v_result ? 'skipped' AND jsonb_typeof(v_result->'skipped') = 'string' THEN
          v_skipped := v_skipped + 1;
          v_skip_reasons := jsonb_set(v_skip_reasons, ARRAY[v_result->>'skipped'],
    ...
    RAISE NOTICE 'Lote % -> %: total=%, ok=%, skipped=%, errors=%',
  ...
  VALUES ('backfill', v_total, NULL, NULL, NULL,
    jsonb_build_object('scope','aceite_financeiro_backfill_retry',
      'function','process_proposta_aceita','batch_size', v_batch_size, 'batches', v_batch_num,
      'ok', v_ok, 'skipped', v_skipped, 'errors', v_errors,
      'skip_reasons', v_skip_reasons, 'error_samples', v_error_samples,
      'duration', (clock_timestamp()-v_t0)::text)::text);
END;
$backfill$;