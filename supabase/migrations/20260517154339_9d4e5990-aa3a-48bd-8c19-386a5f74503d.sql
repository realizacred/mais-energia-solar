...
BEGIN
  ...
  -- Procura na categoria 'Identidade' ou no campo customizado 'cap_identidade'
  SELECT EXISTS (
    SELECT 1 FROM public.project_documents 
    WHERE projeto_id = p_projeto_id 
    AND (categoria ILIKE '%identidade%' OR categoria ILIKE '%rg%' OR categoria ILIKE '%cnh%')
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.project_documents 
    WHERE projeto_id = p_projeto_id 
    AND (categoria ILIKE '%endereço%' OR categoria ILIKE '%residencia%')
    AND is_deleted IS NOT TRUE
  ) OR EXISTS (
    SELECT 1 FROM public.deal_custom_field_values v
    JOIN public.deal_custom_fields f ON f.id = v.field_id
  ...
  -- 3. Verificar Conta de Energia (UC)
  SELECT EXISTS (
    SELECT 1 FROM public.project_documents 
    WHERE projeto_id = p_projeto_id 
    AND (categoria ILIKE '%conta%' OR categoria ILIKE '%energia%' OR categoria ILIKE '%unidade%')
    AND is_deleted IS NOT TRUE
  ) INTO v_has_bill;
...
-- Atualizar get_project_operational_data para corrigir filtros e incluir docs
