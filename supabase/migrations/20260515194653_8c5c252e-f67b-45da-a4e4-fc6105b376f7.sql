...
BEGIN
  ...
  -- Enum proposta_nativa_status usa valores em INGLÊS: draft|generated|sent|accepted|rejected|expired|excluida|arquivada
  v_status := CASE
  ...
  -- propostas_nativas.status é TEXT (valores em PT: 'gerada'/'rascunho'/...).
  IF v_is_official THEN
    UPDATE public.propostas_nativas
    SET status = 'gerada',
...
-- Também corrige create_proposta_nativa_atomic: status do enum em proposta_versoes deve ser 'draft'.
...
BEGIN
  ...
  -- propostas_nativas.status é TEXT em PT
  ...
  -- proposta_versoes.status é ENUM em INGLÊS
