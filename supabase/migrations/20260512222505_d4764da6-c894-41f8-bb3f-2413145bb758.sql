...
-- ============================================================
-- PASSO 1.A — Backfill de colunas NULL a partir do snapshot
-- ============================================================
UPDATE proposta_versoes
SET payback_meses = NULLIF(snapshot->>'payback_meses','')::int
WHERE payback_meses IS NULL
  AND snapshot ? 'payback_meses'
  AND NULLIF(snapshot->>'payback_meses','') IS NOT NULL;

UPDATE proposta_versoes
SET consumo_mensal = NULLIF(snapshot->'ucs'->0->>'consumo_mensal_kwh','')::numeric
WHERE consumo_mensal IS NULL
  AND snapshot->'ucs'->0 ? 'consumo_mensal_kwh'
  AND NULLIF(snapshot->'ucs'->0->>'consumo_mensal_kwh','') IS NOT NULL;

UPDATE proposta_versoes
SET tarifa_distribuidora = NULLIF(snapshot->'ucs'->0->>'tarifa_distribuidora','')::numeric
WHERE tarifa_distribuidora IS NULL
  AND snapshot->'ucs'->0 ? 'tarifa_distribuidora'
  AND NULLIF(snapshot->'ucs'->0->>'tarifa_distribuidora','') IS NOT NULL;

-- ============================================================
-- PASSO 1.B — RPC atualizada: deriva colunas do snapshot
-- ============================================================
...
BEGIN
  ...
  -- Derivar valores canônicos do snapshot
