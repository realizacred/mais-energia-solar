...
-- 1) Add deal_id column to propostas_nativas (direct link to deals table)
ALTER TABLE public.propostas_nativas
ADD COLUMN IF NOT EXISTS deal_id UUID REFERENCES public.deals(id);

-- 2) Create index for fast lookups
CREATE INDEX IF NOT EXISTS idx_propostas_nativas_deal_id ON public.propostas_nativas(deal_id);

-- 3) Update RPC to store deal_id directly in propostas_nativas
...
BEGIN
  ...
  -- Insert proposta WITH deal_id directly
