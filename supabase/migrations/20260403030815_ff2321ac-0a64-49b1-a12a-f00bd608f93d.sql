...
BEGIN
  ...
  GET DIAGNOSTICS v_count = ROW_COUNT;
  result := result || jsonb_build_object('comissoes', v_count);
...
BEGIN
  ...
  GET DIAGNOSTICS v_count = ROW_COUNT;
  result := result || jsonb_build_object('sync_logs', v_count);

  DELETE FROM solar_market_proposals WHERE tenant_id = p_tenant_id;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  result := result || jsonb_build_object('sm_proposals', v_count);

  DELETE FROM solar_market_projects WHERE tenant_id = p_tenant_id;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  result := result || jsonb_build_object('sm_projects', v_count);

  DELETE FROM solar_market_clients WHERE tenant_id = p_tenant_id;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  result := result || jsonb_build_object('sm_clients', v_count);
