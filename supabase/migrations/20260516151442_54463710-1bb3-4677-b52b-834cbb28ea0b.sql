...
BEGIN
  ...
  -- 2. Fetch current state of the proposal to be deleted
  SELECT 
    p.deal_id, 
    p.projeto_id, 
    p.is_principal, 
  ...
  -- 3. Atomic Soft Delete the proposal
  UPDATE propostas_nativas
     SET status = 'excluida',
         deleted_at = now(),
         is_principal = false, -- Reset principal flag
         updated_at = now()
  ...
  -- 5. Find Next Best Proposal to be Principal and provide values
  -- Priority: status rank (aceita > enviada > gerada > others), then most recent
  ...
  IF v_next_proposta_id IS NOT NULL THEN
    -- Mark the next one as principal
    UPDATE propostas_nativas
  ...
  -- 6. Synchronize Deal & Project values
  -- Note: if v_next_proposta_id is NULL, v_new_value and v_new_kwp remain 0
  ...
  IF v_deal_id IS NOT NULL OR v_projeto_id IS NOT NULL THEN
    IF v_next_proposta_id IS NULL THEN
       ...
       -- Remove from non-commercial pipelines
  ...
  INSERT INTO audit_logs (
    ...
    jsonb_build_object(
      ...
      'new_synced_kwp', v_new_kwp,
      'checklists_deleted', v_deleted_checklists,
      'pipelines_removed', v_removed_pipelines
    ),
    v_tenant_id
  );
