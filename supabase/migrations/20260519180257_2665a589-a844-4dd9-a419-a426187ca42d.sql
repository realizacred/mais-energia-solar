CREATE OR REPLACE FUNCTION public.create_proposta_nativa_atomic_v2(p_titulo text, p_lead_id uuid, p_projeto_id uuid, p_deal_id uuid, p_origem text, p_potencia_kwp numeric, p_valor_total numeric, p_snapshot jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (v_tenant_id, COALESCE(NULLIF(p_titulo,''), 'Proposta sem título'), v_cliente_id, p_lead_id, v_projeto_id, p_deal_id, 'draft', p_origem, v_user_id)
