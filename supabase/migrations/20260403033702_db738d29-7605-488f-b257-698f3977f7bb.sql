...
-- Recriar reset_migrated_data com TODAS as FKs na ordem correta
...
BEGIN
  ...
  DELETE FROM projeto_etiqueta_rel WHERE tenant_id = p_tenant_id;
...
-- reset_tenant_data reutiliza reset_migrated_data + deleta SM
CREATE OR REPLACE FUNCTION reset_tenant_data(p_tenant_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  PERFORM reset_migrated_data(p_tenant_id);
