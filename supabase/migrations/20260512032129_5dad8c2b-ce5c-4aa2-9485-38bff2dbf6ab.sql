...
BEGIN
  ...
  -- ── RB-60: resolver consultor por precedência (deal → lead → user atual) ──
  SELECT COALESCE(
    ...
    (SELECT c.id FROM consultores c
      WHERE c.user_id = auth.uid() AND c.ativo = true LIMIT 1)
  ) INTO v_consultor_id;
  ...
  -- ── RB-60: gravar consultor_id + created_by na proposta ──
