...
BEGIN
    ...
    FROM (
        ...
        LEFT JOIN propostas_nativas p ON p.id = eel.entity_id AND eel.entity_type = 'proposta'
        LEFT JOIN projetos pr ON pr.id = eel.entity_id AND eel.entity_type = 'projeto'
        LEFT JOIN clientes c ON c.id = eel.entity_id AND eel.entity_type = 'cliente'
        WHERE eel.tenant_id = p_tenant_id 
          AND eel.source = 'solarmarket'
          AND (
            (eel.entity_type = 'proposta' AND p.id IS NULL) OR
            (eel.entity_type = 'projeto' AND pr.id IS NULL) OR
            (eel.entity_type = 'cliente' AND c.id IS NULL)
          )
    ) as broken;
    ...
    v_result := jsonb_build_object(
        ...
        'status', CASE WHEN v_total_staging = v_promoted_propostas AND v_total_staging > 0 THEN 'concluded' ELSE 'in_progress' END,
        'timestamp', now()
    );
