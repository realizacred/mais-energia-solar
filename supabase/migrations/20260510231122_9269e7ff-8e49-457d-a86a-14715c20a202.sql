...
begin
  select json_build_object(
    'total', count(*),
    'sem_proposta', count(*) filter (where matched_projeto_id is null and lead_status_nome is distinct from 'Perdido'),
    'com_proposta', count(*) filter (where matched_projeto_id is not null),
    'sem_projeto',  count(*) filter (where matched_projeto_id is null and lead_status_nome is distinct from 'Perdido'),
