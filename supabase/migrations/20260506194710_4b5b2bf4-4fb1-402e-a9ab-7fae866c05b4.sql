-- Fix reset_migrated_data: deletar vendas_transacional antes de proposta_versoes
-- (FK vendas_transacional.versao_id -> proposta_versoes.id sem ON DELETE CASCADE)
...
DECLARE
  ...
  v_count_vendas_transacional int := 0;
  v_summary text;
BEGIN
  ...
  SELECT COALESCE(array_agg(p.id), '{}'::uuid[]) INTO v_projeto_ids
  FROM public.projetos p
  WHERE p.tenant_id = p_tenant_id AND p.external_source = 'solarmarket';

  SELECT COALESCE(array_agg(DISTINCT d.id), '{}'::uuid[]) INTO v_deal_ids
  FROM public.deals d
  WHERE d.tenant_id = p_tenant_id
    AND (d.projeto_id = ANY(v_projeto_ids) OR COALESCE(d.origem, '') = 'solarmarket');

  SELECT COALESCE(array_agg(pn.id), '{}'::uuid[]) INTO v_proposta_ids
  FROM public.propostas_nativas pn
  WHERE pn.tenant_id = p_tenant_id AND pn.external_source = 'solarmarket';

  SELECT COALESCE(array_agg(pv.id), '{}'::uuid[]) INTO v_versao_ids
  FROM public.proposta_versoes pv
  WHERE pv.proposta_id = ANY(v_proposta_ids);

  SELECT COALESCE(array_agg(pk.id), '{}'::uuid[]) INTO v_kit_ids
  FROM public.proposta_kits pk
  WHERE pk.versao_id = ANY(v_versao_ids);

  UPDATE public.projetos
  SET deal_id = NULL, proposta_id = NULL
  WHERE id = ANY(v_projeto_ids) AND tenant_id = p_tenant_id;

  -- NEW: vendas_transacional referencia proposta_versoes/propostas/deals/projetos sem CASCADE
  WITH deleted AS (
    DELETE FROM public.vendas_transacional
    WHERE tenant_id = p_tenant_id
      AND (
        versao_id = ANY(v_versao_ids)
        OR proposta_id = ANY(v_proposta_ids)
        OR deal_id = ANY(v_deal_ids)
        OR projeto_id = ANY(v_projeto_ids)
      )
    RETURNING id
  )
  SELECT count(*) INTO v_count_vendas_transacional FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_kit_itens WHERE kit_id = ANY(v_kit_ids) RETURNING id)
  SELECT count(*) INTO v_count_kit_itens FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_kits WHERE id = ANY(v_kit_ids) RETURNING id)
  SELECT count(*) INTO v_count_kits FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_versao_ucs WHERE versao_id = ANY(v_versao_ids) RETURNING id)
  SELECT count(*) INTO v_count_ucs FROM deleted;

  DELETE FROM public.proposta_campos_distribuidora WHERE versao_id = ANY(v_versao_ids);

  WITH deleted AS (DELETE FROM public.proposta_views WHERE proposta_id = ANY(v_proposta_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposta_views FROM deleted;

  DELETE FROM public.proposta_comercial WHERE versao_id = ANY(v_versao_ids);

  WITH deleted AS (DELETE FROM public.proposta_cenarios WHERE versao_id = ANY(v_versao_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposta_cenarios FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_envios WHERE versao_id = ANY(v_versao_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposta_envios FROM deleted;

  WITH deleted AS (
    DELETE FROM public.proposta_aceite_tokens
    WHERE proposta_id = ANY(v_proposta_ids) OR versao_id = ANY(v_versao_ids)
    RETURNING id
  )
  SELECT count(*) INTO v_count_proposta_aceite_tokens FROM deleted;

  WITH deleted AS (DELETE FROM public.proposal_events WHERE proposta_id = ANY(v_proposta_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposal_events FROM deleted;

  WITH deleted AS (DELETE FROM public.proposal_followup_queue WHERE proposta_id = ANY(v_proposta_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposal_followup_queue FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_historico WHERE proposta_id = ANY(v_proposta_ids) RETURNING id)
  SELECT count(*) INTO v_count_proposta_historico FROM deleted;

  WITH deleted AS (
    DELETE FROM public.generated_documents
    WHERE tenant_id = p_tenant_id
      AND (projeto_id = ANY(v_projeto_ids) OR deal_id = ANY(v_deal_ids))
    RETURNING id
  )
  SELECT count(*) INTO v_count_generated_documents FROM deleted;

  WITH deleted AS (DELETE FROM public.proposta_versoes WHERE id = ANY(v_versao_ids) RETURNING id)
  SELECT count(*) INTO v_count_versoes FROM deleted;

  WITH deleted AS (DELETE FROM public.propostas_nativas WHERE id = ANY(v_proposta_ids) RETURNING id)
  SELECT count(*) INTO v_count_propostas FROM deleted;

  WITH deleted AS (DELETE FROM public.deal_custom_field_values WHERE deal_id = ANY(v_deal_ids) RETURNING id)
  SELECT count(*) INTO v_count_dcfv FROM deleted;

  WITH deleted AS (DELETE FROM public.deal_stage_history WHERE deal_id = ANY(v_deal_ids) RETURNING id)
  SELECT count(*) INTO v_count_stage_history FROM deleted;

  WITH deleted AS (DELETE FROM public.deal_kanban_projection WHERE deal_id = ANY(v_deal_ids) RETURNING deal_id)
  SELECT count(*) INTO v_count_projection FROM deleted;

  WITH deleted AS (DELETE FROM public.deal_notes WHERE deal_id = ANY(v_deal_ids) RETURNING id)
  SELECT count(*) INTO v_count_deal_notes FROM deleted;

  WITH deleted AS (DELETE FROM public.deal_pipeline_stages WHERE deal_id = ANY(v_deal_ids) RETURNING id)
  SELECT count(*) INTO v_count_deal_pipeline_stages FROM deleted;

  WITH deleted AS (DELETE FROM public.deals WHERE id = ANY(v_deal_ids) RETURNING id)
  SELECT count(*) INTO v_count_deals FROM deleted;

  WITH deleted AS (DELETE FROM public.projetos WHERE id = ANY(v_projeto_ids) RETURNING id)
  ...
  WITH deleted AS (
    ...
    WHERE tenant_id = p_tenant_id AND external_source = 'solarmarket'
    RETURNING id
  )
  SELECT count(*) INTO v_count_clientes FROM deleted;

  v_summary := format(
    'Deletados: %s clientes, %s projetos, %s deals, %s propostas, %s versões, %s vendas transacional, %s vínculos externos.',
    v_count_clientes, v_count_projetos, v_count_deals, v_count_propostas,
    v_count_versoes, v_count_vendas_transacional, v_count_external_links
  );

  IF to_regclass('public.sm_projeto_funis_raw') IS NOT NULL THEN
    EXECUTE 'TRUNCATE TABLE public.sm_projeto_funis_raw';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object(
      ...
      'vendas_transacional', v_count_vendas_transacional,
