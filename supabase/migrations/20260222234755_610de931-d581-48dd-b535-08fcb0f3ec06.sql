...
-- Add 'criado' as first value in the enum
ALTER TYPE public.projeto_status ADD VALUE IF NOT EXISTS 'criado' BEFORE 'aguardando_documentacao';

-- Update the RPC to use 'criado' for new projects
...
BEGIN
  ...
  -- If project exists, pull cliente_id from it
  IF v_projeto_id IS NOT NULL THEN
    SELECT proj.cliente_id INTO v_cliente_id
    FROM projetos proj WHERE proj.id = v_projeto_id;
  END IF;

  -- If still no cliente_id, try to create/find from params
  IF v_cliente_id IS NULL
     AND p_cliente_nome IS NOT NULL AND btrim(p_cliente_nome) <> ''
  ...
  -- If no projeto_id, create one with status 'criado'
  IF v_projeto_id IS NULL THEN
    INSERT INTO projetos (tenant_id, lead_id, cliente_id, codigo, status)
    VALUES (v_tenant_id, p_lead_id, v_cliente_id, p_titulo, 'criado')
    RETURNING id INTO v_projeto_id;
  END IF;

  -- If no deal_id, try to find from projeto
