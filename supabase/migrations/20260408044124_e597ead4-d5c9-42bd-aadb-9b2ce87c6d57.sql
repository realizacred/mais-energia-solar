...
-- RPC: reset_project_area
-- Deletes ALL project-area data (clients, projects, deals, proposals, etc.)
-- but PRESERVES leads, configurations, SM staging data, and system settings.
-- Resets code sequences for clients, projects, and proposals.
...
BEGIN
  ...
  -- 2. proposta_aceite_tokens (via proposta_versoes FK)
  DELETE FROM proposta_aceite_tokens WHERE proposta_versao_id IN (
  ...
  -- 29. Reset SM migration flags (so data can be re-migrated)
