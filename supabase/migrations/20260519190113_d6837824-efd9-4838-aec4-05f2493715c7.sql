...
DECLARE
  ...
  v_won_by uuid;
BEGIN
  SELECT to_jsonb(d.*), d.pipeline_id, d.customer_id, d.owner_id, d.won_by
  ...
  -- Agrega nomes de usuários que moveram etapas OU que marcaram o ganho
  SELECT coalesce(jsonb_object_agg(user_id, nome), '{}'::jsonb)
    INTO v_user_names
  FROM public.profiles
  WHERE user_id IN (
    SELECT DISTINCT moved_by FROM public.deal_stage_history WHERE deal_id = _deal_id AND moved_by IS NOT NULL
    UNION
