...
-- Drop old overloaded versions to avoid ambiguity
DROP FUNCTION IF EXISTS public.proposal_create_version(UUID, UUID, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT);
...
CREATE OR REPLACE FUNCTION public.proposal_create_version(
  ...
  p_payback_meses INT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.proposal_create_version IS 'Backend-driven versioning: branches or updates proposal version. Supports edge function unification.';
