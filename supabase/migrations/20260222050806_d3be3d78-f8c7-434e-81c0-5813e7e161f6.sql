...
-- Atomic RPC: create proposta nativa with guaranteed projeto existence
...
BEGIN
  ...
  -- 2) Ensure projeto exists
  IF p_projeto_id IS NOT NULL THEN
    -- Verify it exists AND belongs to same tenant
    SELECT id INTO v_projeto_id
    FROM projetos
    WHERE id = p_projeto_id AND tenant_id = v_tenant_id;

    IF v_projeto_id IS NULL THEN
      RAISE EXCEPTION 'Projeto % não encontrado ou não pertence ao tenant', p_projeto_id;
    END IF;
  ELSE
    -- Create a minimal projeto row
