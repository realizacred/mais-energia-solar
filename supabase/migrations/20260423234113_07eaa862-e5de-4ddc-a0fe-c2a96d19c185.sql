...
BEGIN
  ...
  WITH novos_clientes AS (
    INSERT INTO clientes (
      ...
      cep, rua, numero, bairro, cidade, estado, complemento, observacoes,
    ...
    SELECT
      ...
      COALESCE(
        NULLIF(regexp_replace(COALESCE(r.payload->>'phone',''), '[^0-9]', '', 'g'), ''),
        '00000000000'
      ),
      NULLIF(TRIM(r.payload->>'email'), ''),
      NULLIF(regexp_replace(COALESCE(r.payload->>'cpf_cnpj', r.payload->>'document', ''), '[^0-9]', '', 'g'), ''),
      NULLIF(regexp_replace(COALESCE(r.payload->>'zip_code', r.payload->>'cep', ''), '[^0-9]', '', 'g'), ''),
      NULLIF(TRIM(COALESCE(r.payload->>'street', r.payload->>'address')), ''),
      ...
      NULLIF(TRIM(r.payload->>'observation'), ''),
      'solarmarket',
      (r.payload->>'id'),
      'solar_market',
      true
