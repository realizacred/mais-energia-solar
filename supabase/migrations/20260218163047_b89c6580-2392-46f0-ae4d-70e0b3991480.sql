-- Fix: get_irradiance_for_simulation was STABLE but does INSERT (cache)
-- PostgreSQL blocks INSERT in STABLE functions. Change to VOLATILE.

CREATE OR REPLACE FUNCTION public.get_irradiance_for_simulation(_version_id uuid, _lat numeric, _lon numeric, _radius_deg numeric DEFAULT 0.5, _method text DEFAULT 'nearest'::text)
 RETURNS json
 LANGUAGE plpgsql
