...
DECLARE
  ...
  v_pipelines int := 0;
  v_pipeline_stages int := 0;
BEGIN
  -- 0a. proposta_views (FK to proposta_aceite_tokens)
  DELETE FROM proposta_views WHERE token_id IN (
    SELECT id FROM proposta_aceite_tokens WHERE tenant_id = p_tenant_id
  );

  -- 0b. proposta_envios (FK to proposta_aceite_tokens and proposta_versoes)
  ...
  -- 28. pipeline_stages (must come before pipelines due to FK)
  DELETE FROM pipeline_stages WHERE pipeline_id IN (
    SELECT id FROM pipelines WHERE tenant_id = p_tenant_id
  );
  GET DIAGNOSTICS v_pipeline_stages = ROW_COUNT;

  -- 29. pipelines
  DELETE FROM pipelines WHERE tenant_id = p_tenant_id;
  GET DIAGNOSTICS v_pipelines = ROW_COUNT;

  -- 30. projeto_etapas (child of projeto_funis)
  DELETE FROM projeto_etapas WHERE funil_id IN (
    SELECT id FROM projeto_funis WHERE tenant_id = p_tenant_id
  );

  -- 31. projeto_funis
  DELETE FROM projeto_funis WHERE tenant_id = p_tenant_id;

  -- 32. deal_pipeline_stages
  DELETE FROM deal_pipeline_stages WHERE tenant_id = p_tenant_id;

  -- 33. Reset counters
  UPDATE tenants
  SET cliente_code_seq = 0,
      projeto_num_seq = 0,
      proposta_num_seq = 0
  WHERE id = p_tenant_id;

  -- 34. Reset SM migration flags
  UPDATE solar_market_clients SET migrated = false WHERE tenant_id = p_tenant_id;
  UPDATE solar_market_projects SET migrated = false WHERE tenant_id = p_tenant_id;
  UPDATE solar_market_proposals SET migrated = false WHERE tenant_id = p_tenant_id;
