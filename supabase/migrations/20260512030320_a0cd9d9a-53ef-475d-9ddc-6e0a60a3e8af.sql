...
BEGIN
  ...
  FROM (
    ...
    SELECT
      COALESCE(c.nome, pr.nome, 'Sem vendedor')          AS vendedor,
      b.resolved_id                                      AS vendedor_id,
      COUNT(*)                                           AS total,
    ...
    FROM base b
    LEFT JOIN consultores c ON c.id      = b.resolved_id
    LEFT JOIN profiles    pr ON pr.user_id = b.resolved_id
    GROUP BY COALESCE(c.nome, pr.nome, 'Sem vendedor'), b.resolved_id
    ORDER BY total DESC
  ) r;
