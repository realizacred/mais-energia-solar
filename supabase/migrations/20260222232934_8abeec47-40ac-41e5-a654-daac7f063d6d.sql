-- Fix: create_proposta_nativa_atomic uses p.id instead of p.user_id to resolve tenant
-- This causes "Tenant não encontrado" for ALL users because profiles.id ≠ auth.uid()
...
DECLARE
  ...
  v_cliente_id uuid;
BEGIN
  -- Resolve tenant from auth user (FIX: use user_id, not id)
  ...
  -- Create/find client if name+phone provided
