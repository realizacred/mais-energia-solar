CREATE OR REPLACE FUNCTION public.create_proposta_nativa_atomic(p_titulo text, p_lead_id uuid DEFAULT NULL::uuid, p_projeto_id uuid DEFAULT NULL::uuid, p_deal_id uuid DEFAULT NULL::uuid, p_origem text DEFAULT 'native'::text, p_potencia_kwp numeric DEFAULT 0, p_valor_total numeric DEFAULT 0, p_snapshot jsonb DEFAULT '{}'::jsonb, p_cliente_nome text DEFAULT NULL::text, p_cliente_telefone text DEFAULT NULL::text, p_cliente_email text DEFAULT NULL::text, p_cliente_cpf_cnpj text DEFAULT NULL::text, p_cliente_empresa text DEFAULT NULL::text, p_cliente_cep text DEFAULT NULL::text, p_cliente_estado text DEFAULT NULL::text, p_cliente_cidade text DEFAULT NULL::text, p_cliente_rua text DEFAULT NULL::text, p_cliente_numero text DEFAULT NULL::text, p_cliente_bairro text DEFAULT NULL::text, p_cliente_complemento text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ── RB-60: resolver projeto pré-existente do deal ANTES de criar novo ──
  IF v_deal_id IS NOT NULL AND v_projeto_id IS NULL THEN
    SELECT projeto_id INTO v_projeto_id
    FROM deals WHERE id = v_deal_id;
  END IF;

  -- ── RB-60: consultor herdado de deal/lead/projeto. NUNCA auth.uid() ──
  -- (gerentes/admins geram propostas, mas não são consultores do projeto)
  SELECT COALESCE(
  ...
  IF v_cliente_id IS NULL AND v_deal_id IS NOT NULL THEN
    SELECT d.customer_id INTO v_cliente_id
    FROM deals d WHERE d.id = v_deal_id;
  END IF;
  ...
  -- ── RB-60: consultor_id herdado (deal/lead); created_by = quem gerou (gerente/admin) ──
