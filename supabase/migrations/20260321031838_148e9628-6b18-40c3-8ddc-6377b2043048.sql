-- Fix default percentual divergence (2.0 → 3.0)
ALTER TABLE comissoes ALTER COLUMN percentual_comissao SET DEFAULT 3.0;

-- Auto-commission trigger on propostas_nativas status change to 'aceita'
...
-- Create the trigger
CREATE TRIGGER trg_proposta_aceita_gerar_comissao
AFTER UPDATE ON propostas_nativas
FOR EACH ROW
EXECUTE FUNCTION trg_proposta_aceita_comissao();